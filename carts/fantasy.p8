...
function _update()
  ...
  elseif globalgamemode == 3 then update_heroes_dead()
  end
end
...
function _draw()
  ...
  elseif globalgamemode == 3 then draw_heroes_dead()
  end
...
function update_game_title()
    ...
    if (colorcounter < 4) then
        color = 0
    elseif (colorcounter == 4) then
        color = 1
    elseif (colorcounter == 8) then
        color = 2
    elseif (colorcounter == 10) then
        color = 1
    elseif (colorcounter == 14) then
        color = 0
        colorcounter = 0
    end

    keypressWait -= 1
    if (keypressWait <= 0) then
...
function draw_game_title()
  ...
  _gametitle = "fASTER fANTASY"
  print(_gametitle,hcenter(_gametitle),vcenter(_gametitle) - 20,color)
  print(_gametitle,hcenter(_gametitle),vcenter(_gametitle)-19,12)

  _gametitle = "eXTRAlIFE eDITION"
  print(_gametitle,hcenter(_gametitle) ,vcenter(_gametitle) - 4,color)
  print(_gametitle,hcenter(_gametitle),vcenter(_gametitle)- 3,12)

  _pressabutton = "press a button"
  print(_pressabutton,hcenter(_pressabutton),vcenter(_pressabutton) + 24, 6)

  -- drawsprites
  spr(sprite1.sprite, sprite1.x, sprite1.y)
end
...
function generate_next_wave(deleteactors)
    ...
    elseif (globalgamewave == 3) then
        enemyone = create_enemy(60, "cleric", true, globalinterval)
        enemytwo = create_enemy(70, "ranger", true, globalinterval)
        enemythree = create_enemy(80, "warrior", true, globalinterval)
        enemyfour = create_enemy(90, "nothing", false, globalinterval)
    ...
    elseif (globalgamewave == 5) then
        enemyone = create_enemy(60, "wizard", true, globalinterval)
        enemytwo = create_enemy(70, "ranger", true, globalinterval)
    ...
    elseif (globalgamewave == 6) then
        enemyone = create_enemy(60, "cleric", true, globalinterval)
        enemytwo = create_enemy(70, "warrior", true, globalinterval)
    ...
    elseif (globalgamewave == 7) then
        ...
        enemytwo = create_enemy(70, "ranger", true, globalinterval)
        enemythree = create_enemy(80, "warrior", true, globalinterval)
        enemyfour = create_enemy(90, "wizard", true, globalinterval)
    ...
    elseif (globalgamewave == 8) then
        ...
        -- set up next loop
        globalinterval -= 20 -- cuts 15 frames / .5 seconds
...
function create_all_heroes()
    heroleft = make_actor(globalherostartx,60,1,"warrior")
    ...
    heroright = make_actor(globalherostartx,80,1,"cleric")
...
function create_enemy(ycoord, type, visibility, baseinterval)
    ...
    _newenemy.interval = create_minimal_viable_interval(baseinterval)
    _newenemy.nextattacktime = _newenemy.interval
    flipenemymovement(_newenemy)
    _newenemy.state = "ready"
    return _newenemy
end

function create_minimal_viable_interval(baseinterval)

    tempinterval = rnd(baseinterval) + 40
    if (tempinterval < 60) then
        return create_minimal_viable_interval(baseinterval)
    else
        return tempinterval
    end
end
...
function set_class_values(actor)
    ...
    if actor.class == "cleric" then
        actor.ability = "heal"
        actor.healvalue = 1
        actor.attackspeed = 3.5
        actor.recoverspeed = -.5 -- was .75
        actor.maxhealth = 3
...
function update_game_main()
  ...
  if (check_for_heroes_dead() == true) then
      globalgamemode = 3
      keypressWait = 60
      return
  end
...
function check_enemy_state(actor)
    ...
    if globalbattletime >= actor.nextattacktime and actor.state == "ready" and actor.visible == true and actor.currenthealth > 0 then
        actor.nextattacktime += actor.interval
        actor.state = "charging"
        actor.dx = actor.attackspeed
    end
end
...
function trigger_attack(actor)
    ...
    elseif (actor.type == "enemy") then
        ...
        elseif (actor.ability == "ranged") then
            sfx(1, -1)
            if (check_for_heroes_dead() == false) then
                livingtargets = find_living_targets(heroleft, heroup, heroright, herodown)
                target = find_farthest_target(livingtargets)
                target.currenthealth -= 1
            end
...
function draw_game_main()
  ...
  --print("  wave: "..globalgamewave,0,120,7)

  _score ="  score: "..currentscore
  print(_score,hcenter(_score),20,7)
  print("hIGH sCORE: "..highscore,0,120,7)
end
...
function draw_game_debug()
    ...
    --print("t "..flr(globaltime / 30),64,110,7)
    --print("s "..enemyone.interval,64,120,7)
...
function draw_heroes_dead()
    ...
    map(0, 0, mapx, 0, 32, 32, 0)
    _msg = "  your heroes died..."
    print(_msg,hcenter(_msg),10,6)
    _score ="  score: "..globalgamewave
    print(_score,hcenter(_score),20,6)
...
__gfx__

...

...

...

__map__

